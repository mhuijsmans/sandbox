...
void web(int fd, int hit)
{
	...
	for (i = 4; i < BUFSIZE; i++)
	{ /* null terminate after the second spaceto ignore extra stuff */
		if (buffer[i] == ' ')
	...
	{/* check for illegal parent directory use.. */
		...
		{
			log(SORRY, "Parent directory (..) path names not supported", buffer,
					fd);
		}
	}
...
//	if (chdir(argv[2]) == -1)
	...
	if (setsockopt(listenfd,SOL_SOCKET,SO_REUSEADDR,  &so_reuseaddr,   sizeof so_reuseaddr) < 0) {
		log(ERROR, "system call", "setsockopt", 0);
	}

	port = 8181; // atoi(argv[1]);
