...
public class BaseClass<T> implements Factory<T> {
	...
	private final static Logger LOGGER = Logger.getLogger(FuncJavaProxy.class.getName());	
	
	private final Class<T> cls;
	
	public BaseClass(final Class<T> cls) {
		LOGGER.info("ctor() with cls " + cls.getName()+" this="+this);
		this.cls = cls;
	}

	@Override
	public T get() {
		LOGGER.info("get() with cls "+cls + " this="+this);
		try {
